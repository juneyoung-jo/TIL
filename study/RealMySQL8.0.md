...
- 실행 계획 분석
  ...
  - key 칼럼
    ...
    - type 칼럼이 index_merge가 아닌 경우에는 반드시 테이블 하나당 하나의 인덱스만 이요할 수 있다.
  - key_len 칼럼
    - 쿼리를 처리하기 위해 다중 칼럼으로 구성된 인덱스에서 몇 개의 칼럼까지 사용했는지 알려주는 정보
    - 인덱스의 각 레코드에서 몇 바이트까지 사용했는지 알려주는 정보
  - ref 칼럼
    - 접근방법이 ref면 참조 조건(Equal 비교 조건)으로 어떤 값이 제공됐는지 보여준다.
      - 상수값은 const
      - 다른 테이블의 칼럼값이면 그 테이블명과 칼럼명이 표시됨
    - func 라고 표시될 때는 주의해야 함.
      - 참조용으로 사용되는 값을 그대로 사용한 것이 아니라 콜레이션 변환이나 값 자체의 연산ㅇ르 거쳐서 참조됐다는 것을 의미함.
  - rows 칼럼
    - 실행 계획의 효율성 판단을 위해 예측했던 레코드 건수를 보여준다.
    - MySQL 옵티마이저가 예측한 값은 틀릴 가능성이 높다. 대략적인 수치임.
  - filtered 칼럼
    - 필터링되고 남은 레코드의 비율을 의미함.
    - filterd 칼럼에 표시되는 값이 얼마나 정확히 예측될 수 있느냐에 따라 조인의 성능이 달라짐.
  - Extra 칼럼
    - 성능에 관련된 중요한 내용이 표기됨.
